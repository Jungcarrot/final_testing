...
window.reportComment = async function (commentId) {
    ...
    try {
        ...
        if (commentSnapshot.exists()) {
            ...
            const reportedUserID = commentData.commenter;  // 신고당한 사람의 UID

            // 신고 데이터 생성
            const reportData = {
                commentID: commentId,
                reporterID: loggedUserId,  // 신고한 사람의 UID
                reportedUserID: reportedUserID,  // 신고당한 사람의 UID
                reportReason,
        ...
        function editPost() {
            ...
            window.location.href = `findWrite.html?pid=${postId}&edit=true&details=${encodeURIComponent(postDetails)}`; // 연결된 부분 수정
        }
